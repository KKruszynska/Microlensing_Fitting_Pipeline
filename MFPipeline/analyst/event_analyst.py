...
class EventAnalyst(Analyst):
    ...
    def run_lc_analyst(self):
        ...
        if (lc_quality_status):
            self.log.info("Event Analyst: Lc quality procedures finished succesfully.")
        else:
    ...
    def run_cmd_analyst(self):
        ...
        for dictionary in self.config["cmd_analyst"]["catalogues"]:
            ...
            if (plot_status):
                self.log.info("Event Analyst: CMD plot created successfully for %s."%catalogue)
            else:
                self.log.error("Event Analyst: Problems while creating CMD plot for %s." % catalogue)
